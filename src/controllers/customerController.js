...
export const CustomerController = {
  ...
  async create(req, res) {
    try {
      ...
      if (!email || !email.includes("@")) {
        return res.status(400).json({ error: "Email harus valid dan mengandung karakter '@'" });
      }

      // Validasi Phone
      if (!phone || phone.length < 10) {
        return res.status(400).json({ error: "Nomor telepon (phone) wajib diisi minimal 10 karakter" });
      }
  ...
  async update(req, res) {
    try {
      ...
      // Validasi Email pada body
      if (email !== undefined && !email.includes("@")) {
        return res.status(400).json({ error: "Email harus valid dan mengandung karakter '@'" });
      }

      // Validasi Phone pada body
  ...
  // Controller endpoint total pelanggan
